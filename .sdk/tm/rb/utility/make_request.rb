...
module EmailValidationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = EmailValidationResponse.new({})
    ...
    if fetch_err
      ...
      response = EmailValidationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = EmailValidationResponse.new(fetched)
    else
