...
// EmailValidation SDK pipeline test
//
...
{
    private static function utility(): EmailValidationUtility
    {
        return new EmailValidationUtility();
    }
    ...
    private static function ctx(array $over = []): EmailValidationContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new EmailValidationContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new EmailValidationOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof EmailValidationError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new EmailValidationOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = EmailValidationMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new EmailValidationOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = EmailValidationMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new EmailValidationOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = EmailValidationMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EmailValidationSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = EmailValidationMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        ...
        [, $err] = EmailValidationMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new EmailValidationSpec([]),
            'response' => null,
            'result' => new EmailValidationResult([]),
        ]);
        [, $err] = EmailValidationMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new EmailValidationSpec(['step' => 's']),
            'response' => new EmailValidationResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new EmailValidationResult([]),
        ]);
        [, $err] = EmailValidationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new EmailValidationSpec(['step' => 's']),
            'response' => new EmailValidationResponse(self::resp(200, ['v' => 1])),
            'result' => new EmailValidationResult([]),
        ]);
        [, $err] = EmailValidationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new EmailValidationSpec(['step' => 's']),
            'response' => new EmailValidationResponse(self::resp(200, ['v' => 2])),
            'result' => new EmailValidationResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        EmailValidationMakeResponse::call($ctx);
    ...
    {
        $preset = new EmailValidationResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new EmailValidationSpec([]),
            'response' => new EmailValidationResponse([]),
            'result' => new EmailValidationResult([]),
        ...
        [$got, $err] = EmailValidationMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new EmailValidationResult([])]);
        [, $err] = EmailValidationMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new EmailValidationSpec([]), 'result' => null]);
        [, $err] = EmailValidationMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EmailValidationSpec(['step' => 's']),
            'result' => new EmailValidationResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new EmailValidationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EmailValidationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EmailValidationSpec(['step' => 's']),
            'result' => new EmailValidationResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new EmailValidationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EmailValidationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EmailValidationResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new EmailValidationSpec([]),
            'result' => new EmailValidationResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = EmailValidationMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EmailValidationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new EmailValidationError('boom', 'boom');
        $utility->fetcher = function (EmailValidationContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EmailValidationSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = EmailValidationMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (EmailValidationContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = EmailValidationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(EmailValidationResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        EmailValidationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (EmailValidationContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EmailValidationSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = EmailValidationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new EmailValidationResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new EmailValidationSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = EmailValidationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = EmailValidationMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = EmailValidationMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new EmailValidationResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, EmailValidationDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new EmailValidationResult(['ok' => false])]);
        $this->expectException(EmailValidationError::class);
        EmailValidationDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new EmailValidationResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', EmailValidationMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        EmailValidationMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new EmailValidationBaseFeature();
        $b = new EmailValidationBaseFeature();
        EmailValidationFeatureAdd::call($ctx, $a);
        EmailValidationFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): EmailValidationBaseFeature
    {
        $f = new EmailValidationBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        EmailValidationFeatureAdd::call($ctx, self::named_feature('a'));
        EmailValidationFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        EmailValidationFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        EmailValidationFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        EmailValidationFeatureAdd::call($ctx, $replace);
        ...
        EmailValidationFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): EmailValidationContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new EmailValidationSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EmailValidationPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        EmailValidationPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new EmailValidationResponse(['headers' => null]),
            'result' => new EmailValidationResult([]),
        ]);
        EmailValidationResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        EmailValidationResultBody::call($ctx);
