...
// EmailValidation SDK feature test
//
...
class FtCtrl extends EmailValidationControl
{
...
class FtClient
{
    ...
    private EmailValidationUtility $utility;

    public function __construct(EmailValidationUtility $utility, array $options)
    {
    ...
    public function get_utility(): EmailValidationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (EmailValidationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public EmailValidationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new EmailValidationUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (EmailValidationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = EmailValidationConfig::make_feature($name);
    ...
    private static function buildUrl(EmailValidationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof EmailValidationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new EmailValidationResponse($fetched);
        }
    ...
    private function fail(EmailValidationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (EmailValidationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
