...
class EmailValidationMakeError
{
    public static function call(?EmailValidationContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new EmailValidationContext([], null);
        }
        $op = $ctx->op ?? new EmailValidationOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new EmailValidationResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof EmailValidationError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new EmailValidationError('', $msg, $ctx);
