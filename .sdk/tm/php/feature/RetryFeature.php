...
// EmailValidation SDK retry feature
...
class EmailValidationRetryFeature extends EmailValidationBaseFeature
{
    ...
    public function init(EmailValidationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (EmailValidationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(EmailValidationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(EmailValidationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
