...
boru-driven command-line client **and** interactive REPL for the EmailValidation
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/email-validation-cli)
make build

# 2. See usage (words, entities, env vars)
./email-validation-cli --help

# 3. Provide credentials once, via the environment
export EMAIL_VALIDATION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./email-validation-cli load 1 dns_result            # {id:1} shorthand
./email-validation-cli load '{id:1}' dns_result       # explicit match map

# 5. Override the API base URL for a single call
EMAIL_VALIDATION_BASE=https://api.example.com ./email-validation-cli load 1 dns_result

# 6. No arguments -> interactive REPL
./email-validation-cli
email-validation> load 1 dns_result
email-validation> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/email-validation-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/email-validation-cli load 1 dns_result
   ```
...
```sh
./email-validation-cli load 1 dns_result          # scalar shorthand for {id:1}
./email-validation-cli load '{id:1}' dns_result     # explicit match map
```
...
./email-validation-cli load 1 dns_result
```
...
Run with no arguments to open a REPL (prompt `email-validation>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/email-validation-cli`. |
...
(`./email-validation-cli <expr>`) and interactively (the REPL), and expressions compose the
