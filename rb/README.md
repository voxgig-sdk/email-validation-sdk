# EmailValidation Ruby SDK



The Ruby SDK for the EmailValidation API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
require_relative "EmailValidation_sdk"
client = EmailValidationSDK.new(options)
```
...
```ruby
client = EmailValidationSDK.test(testopts, sdkopts)
```
...
raise a `EmailValidationError` (a `StandardError` subclass), so wrap
...
```
rb/
├── EmailValidation_sdk.rb       -- Main SDK module
...
The main module (`EmailValidation_sdk`) exports the SDK class
