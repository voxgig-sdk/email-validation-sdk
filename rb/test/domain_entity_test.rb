...
class DomainEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = EmailValidationSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set EMAILVALIDATION_TEST_DOMAIN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def domain_basic_setup(extra)
  ...
  entid_env_raw = ENV["EMAILVALIDATION_TEST_DOMAIN_ENTID"]
  ...
  if env["EMAILVALIDATION_TEST_LIVE"] == "TRUE"
    ...
    client = EmailValidationSDK.new(Helpers.to_map(merged_opts))
  end
