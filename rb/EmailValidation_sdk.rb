...
require_relative 'EmailValidation_types'
...
class EmailValidationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = EmailValidationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = EmailValidationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, EmailValidationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    EmailValidationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = EmailValidationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = EmailValidationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = EmailValidationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue EmailValidationError => err
    ...
    if fetched.is_a?(Hash)
      status = EmailValidationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = EmailValidationError.new(
        "graphql_error", "EmailValidationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = EmailValidationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
