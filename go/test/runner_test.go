...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("EMAIL_VALIDATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("EMAIL_VALIDATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("EMAIL_VALIDATION_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.EmailValidationSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.EmailValidationError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.EmailValidationSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.EmailValidationError{Msg: msg, Code: code}
}
