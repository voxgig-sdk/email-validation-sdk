...
func TestDnsResultEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set EMAILVALIDATION_TEST_DNS_RESULT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func dns_resultBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("EMAILVALIDATION_TEST_DNS_RESULT_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"EMAILVALIDATION_TEST_DNS_RESULT_ENTID": idmap,
		"EMAILVALIDATION_TEST_LIVE":      "FALSE",
		"EMAILVALIDATION_TEST_EXPLAIN":   "FALSE",
		"EMAILVALIDATION_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["EMAILVALIDATION_TEST_DNS_RESULT_ENTID"])
