...
const __1 = require("..");
const SDK_NAME = 'EmailValidationSDK';
...
function isRunnable(code) {
    return (/new\s+EmailValidationSDK\b/.test(code) ||
        /\bEmailValidationSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+EmailValidationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/EmailValidationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
