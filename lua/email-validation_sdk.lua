...
require("email-validation_types")
...
local EmailValidationSDK = {}
EmailValidationSDK.__index = EmailValidationSDK
...
EmailValidationSDK._make_feature = _make_feature


function EmailValidationSDK.new(options)
  local self = setmetatable({}, EmailValidationSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function EmailValidationSDK:options_map()
...
function EmailValidationSDK:get_utility()
  return Utility.copy(self._utility)
end


function EmailValidationSDK:get_root_ctx()
  return self._rootctx
end


function EmailValidationSDK:prepare(fetchargs)
...
function EmailValidationSDK:direct(fetchargs)
...
function EmailValidationSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function EmailValidationSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "EmailValidationSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function EmailValidationSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function EmailValidationSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "EmailValidationSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function EmailValidationSDK:DnsResult(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function EmailValidationSDK:Domain(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function EmailValidationSDK:EmailValidate(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function EmailValidationSDK:Generate(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function EmailValidationSDK:Grammar(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function EmailValidationSDK:Ipn(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function EmailValidationSDK:Redact(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function EmailValidationSDK:Ssl(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function EmailValidationSDK:Utility(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function EmailValidationSDK:Whoi(data)
...
function EmailValidationSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = EmailValidationSDK.new(sdkopts)
  sdk.mode = "test"
