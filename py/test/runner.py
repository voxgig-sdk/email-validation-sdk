# EmailValidation SDK test runner
...
class EmailValidationTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            EmailValidationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = EmailValidationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = EmailValidationTestRunner.getenv("EMAIL_VALIDATION_TEST_LIVE")
        override = EmailValidationTestRunner.getenv("EMAIL_VALIDATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = EmailValidationTestRunner.getenv(key)
        ...
        explain = EmailValidationTestRunner.getenv("EMAIL_VALIDATION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if EmailValidationTestRunner._test_control is not None:
            return EmailValidationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                EmailValidationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            EmailValidationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = EmailValidationTestRunner.load_test_control()
...
def load_env_local():
    EmailValidationTestRunner.load_env_local()


def env_override(m):
    return EmailValidationTestRunner.env_override(m)


def entity_data(v):
    return EmailValidationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return EmailValidationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return EmailValidationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return EmailValidationTestRunner.load_test_control()


def live_client_options():
    return EmailValidationTestRunner.live_client_options()


def live_delay_ms():
    return EmailValidationTestRunner.live_delay_ms()
